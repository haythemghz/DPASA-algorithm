"""
Enhanced SDM Run on Cantilever Beam Problem
===========================================

Runs SDM on the Cantilever Beam Design Problem with enhanced population settings:
- Leaders (Np): 150
- Followers (Nf): 1600
- Iterations: 1000
...
from benchmarks.engineering import CantileverBeam
...
def test_problem(problem_class, n_trials=10, max_iter=1000):
    ...
    for trial in range(n_trials):
        ...
        max_violation = np.max(np.maximum(0, constraints)) if constraints.size > 0 else 0.0
...
def main():
    print("ENHANCED CANTILEVER BEAM BENCHMARK (Np=150, Nf=1600, Iter=1000)")
    
    summary = []
    res = test_problem(CantileverBeam, n_trials=10, max_iter=1000)
    summary.append(res)
    ...
    print("-" * 80)
    for s in summary:
        print(f"{s['name']:<20} {CantileverBeam.best_known:<12.6f} {s['best']:<12.6f} {s['mean']:<12.6f} {s['gap_best']:<8.4f}")
    print("="*80)
